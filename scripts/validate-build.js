...
// Explicit prerequisite: this script asserts on the *built* site. `npm run
// verify` / `npm run validate` on a fresh clone must fail here with a clear
// message, not a cryptic ENOENT halfway through.
if (!fs.existsSync(path.join(build, 'index.html'))) {
  console.error(
    'validate-build: no build/ output found.\n' +
      'Run `npm run build` first — this validator checks the built HTML.',
  );
  process.exit(1);
}
...
check(
  'OG image exists',
  fs.existsSync(path.join(build, 'img', 'og-zcohen-nerd.png')),
);
...
for (const [, body] of jsonLd) {
  try {
    ...
    check(
      `JSON-LD ${parsed['@type']} parses`,
      ['WebSite', 'Person'].includes(parsed['@type']),
    );
  } catch (e) {
...
check(
  'sitemap uses apex domain',
  locs.every((u) => u.startsWith('https://zcohen-nerd.com/')),
  locs.find((u) => !u.startsWith('https://zcohen-nerd.com/')),
);
check(
...
check(
  ...
  robots.includes('Sitemap: https://zcohen-nerd.com/sitemap.xml'),
);
check(
...
const legacy = [
  ...
  'href="https://www.zcohen-nerd.com',
];
...
check(
  ...
  plausibleCount === 1,
  `found ${plausibleCount}`,
);
check(
  'Plausible domain is apex',
  indexHtml.includes('data-domain="zcohen-nerd.com"'),
);
...
check(
  'disclosure trigger has aria-controls',
  indexHtml.includes('aria-controls="zc-project-disclosure"'),
);
...
const discBody =
  discStart !== -1 && discEnd > discStart
    ...
    : '';
check(
  'all 8 registry links server-rendered in disclosure',
...
const triggerMatch = indexHtml.match(
...
check(
  ...
  triggerText.startsWith('Ecosystem'),
  `got "${triggerText}"`,
);
check(
  'old shared Projects trigger absent',
  !triggerText.startsWith('Projects'),
);
...
const fsbCardStart =
  fsbTitleIdx === -1 ? -1 : indexHtml.lastIndexOf('<a ', fsbTitleIdx);
const fsbCard =
  fsbCardStart === -1 ? '' : indexHtml.slice(fsbCardStart, fsbTitleIdx);
check(
  'Fusion System Blocks card shows Public Beta',
  fsbCard.includes('Public Beta'),
  fsbTitleIdx === -1 ? 'card not found' : 'pill mismatch',
);
...
check(
  'About links the stable resume PDF',
  aboutHtml.includes(
    'https://portfolio.zcohen-nerd.com/files/zac-cohen-resume.pdf',
  ),
);
check(
  'About resume link identifies PDF',
  /r&#x27;|résumé|resume/i.test(aboutHtml) && aboutHtml.includes('(PDF)'),
);
check(
  'About keeps email link',
  aboutHtml.includes('mailto:zachary@zcohen-nerd.com'),
);
check(
  'About keeps LinkedIn link',
  aboutHtml.includes('linkedin.com/in/zachary-cohen-nerd'),
);

// Homepage: Current Focus replaces the repeated identity section.
const indexVisible = indexHtml.replace(/<script[\s\S]*?<\/script>/g, '');
check(
  'homepage has Current Focus section',
  indexVisible.includes('Current focus'),
);
check(
  'obsolete Who’s-behind heading absent',
...
check(
  'at least three focus items',
  (focusRegion.match(/<h3/g) || []).length >= 3,
);
check(
  'no fixed writing cadence on homepage',
  !indexVisible.includes('six weeks') && !indexVisible.includes('six-week'),
);
...
check(
  '404 links to hub home and portfolio',
  notFound.includes('href="/"') &&
    notFound.includes('https://portfolio.zcohen-nerd.com/'),
);
const hubTitle =
  (indexHtml.match(/<title[^>]*>([^<]+)<\/title>/) || [])[1] || '';
check(
  'hub homepage title under 70 chars',
  hubTitle.length <= 70,
  `${hubTitle.length} chars`,
);
...
check(
  'selected-work sits before the ecosystem',
  workStart !== -1 && ecoStart > workStart,
  `work@${workStart} eco@${ecoStart}`,
);
const heroRegion = workStart !== -1 ? indexHtml.slice(0, workStart) : indexHtml;
const workSection =
  workStart !== -1 && ecoStart > workStart
    ? indexHtml.slice(workStart, ecoStart)
    : '';

// Exactly three flagship cards, each a single whole-card link (no nested <a>).
const workTitles = [...workSection.matchAll(/<h3[^>]*>([\s\S]*?)<\/h3>/g)].map(
  (m) => m[1].replace(/<[^>]+>/g, '').trim(),
);
check(
  'exactly three flagship cards',
  workTitles.length === 3,
  `found ${workTitles.length}: ${workTitles.join(' | ')}`,
);
const FLAGSHIP = {
...
for (const [name, meta] of Object.entries(FLAGSHIP)) {
  ...
  check(
    `status pill for ${name} reads "${meta.status}"`,
    card.includes(`>${meta.status}<`),
    titleIdx === -1 ? 'card not found' : 'status text missing',
  );
}
...
const sparkCard =
  sparkIdx === -1
    ? ''
    : workSection.slice(
        workSection.lastIndexOf('<a ', sparkIdx),
        workSection.indexOf('<a ', sparkIdx) === -1
          ? workSection.length
          : workSection.indexOf('<a ', sparkIdx),
      );
check(
  'SPARK is not labelled Deployed',
  sparkIdx !== -1 && !sparkCard.includes('>Deployed<'),
);
...
const workStatusPills = [
  ...workSection.matchAll(/class="[^"]*workStatus[^"]*"[^>]*>\s*([^<]+?)\s*</g),
].map((m) => m[1].trim());
check(
  'proof-layer status pills present',
  workStatusPills.length === 3,
  `found ${workStatusPills.length}: ${workStatusPills.join(' | ')}`,
);
for (const p of workStatusPills) {
  check(
    `proof-layer status "${p}" is approved hub vocabulary`,
    HUB_APPROVED_STATUS.includes(p),
  );
}
check(
  'proof layer has no drifted status token',
...
check(
  'every card shows a Role line',
  (workSection.match(/>Role</g) || []).length === 3,
);
check(
  'every card shows an Evidence line',
  (workSection.match(/>Evidence</g) || []).length === 3,
);
...
check(
  'three flagship images',
  workImgs.length === 3,
  `found ${workImgs.length}`,
);
for (const img of workImgs) {
  check(
    'flagship image has explicit width+height',
    /\bwidth="\d+"/.test(img) && /\bheight="\d+"/.test(img),
    img.slice(0, 140),
  );
  check(
    'flagship image has meaningful alt text',
    /\balt="[^"]{12,}"/.test(img),
    img.slice(0, 140),
  );
  check(
    'flagship image is lazy-loaded .webp',
    img.includes('loading="lazy"') && /src="[^"]+\.webp"/.test(img),
    img.slice(0, 140),
  );
}
for (const meta of Object.values(FLAGSHIP)) {
  check(
    `flagship asset built: img/work/${meta.asset}`,
    fs.existsSync(path.join(build, 'img', 'work', meta.asset)),
  );
}
...
check(
  'proof layer is curated, not the whole registry',
  leaked.length === 0,
  `leaked: ${leaked.join(', ')}`,
);
...
check(
  'hero primary action points to #selected-work',
  heroRegion.includes('href="#selected-work"'),
);
check(
  'hero secondary action points to #ecosystem',
  heroRegion.includes('href="#ecosystem"'),
);
...
check(
  'registry has the expected 8 canonical entries',
  registryList.length === 8,
  `got ${registryList.length}`,
);
for (const p of registryList) {
  check(
    `ecosystem still renders registry entry: ${p.name}`,
    ecoSection.includes(p.name),
    p.name,
  );
  check(
    `ecosystem still links registry href: ${p.name}`,
    ecoSection.includes(`href="${p.href}"`),
    p.href,
  );
}

// Third-party origin allowlist — the built output must load no cross-origin
// resource other than what src/pages/privacy.md + NOTICE.md declare (Plausible).
// The Cloudflare Web Analytics beacon is edge-injected; the separate
// `--mode live` check (run after deploy) covers the served response.
try {
  require('child_process').execFileSync(
    process.execPath,
    [path.join(__dirname, 'check-third-party-origins.mjs'), '--mode', 'built'],
    {stdio: 'inherit'},
  );
  check('built output: third-party origins on the allowlist', true);
} catch {
  check(
    'built output: third-party origins on the allowlist',
    false,
    'see output above',
  );
}
